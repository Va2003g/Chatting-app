...
import { doc, collection, query, orderBy, onSnapshot, setDoc, Timestamp } from "firebase/firestore";
import { MessageType, UserType } from "@/utils/Types";

const ChatItem = ({ item, index, noBorder, router, currentUser }: { item: UserType; index: number; noBorder: boolean; router: any; currentUser: UserType }) => {
  ...
  return (
    <TouchableOpacity className={`flex-row justify-between mx-4 items-center gap-3 mb-4 pb-2 ${!noBorder && "border-b border-b-neutral-200"}`} onPress={openChatRoom}>
      <Image
